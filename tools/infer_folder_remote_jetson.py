...
def main():
  ...
  test_mode = test_images_dir is not None
  if test_mode and test_images_dir == image_dir:
  ...
  if test_mode and not test_paths:
  ...
  if test_mode:
    ...
    print("press Enter in the terminal for the next test image")
  else:
    print(f"watching: {image_dir}")
    print("drop an image into the watched folder to process it")
    print("press any key after a result to clear the display")
  ...
  try:
    while True:
      ...
      try:
        ...
        if test_paths:
          cv2.waitKey(1)
          input("press Enter for next test image...")
          wait_result = "key"
        else:
        ...
        if not test_paths:
          ...
          continue
      if test_mode or wait_result == "key":
        cv2.imshow(window_name, blank)
        cv2.waitKey(args.poll_ms)
