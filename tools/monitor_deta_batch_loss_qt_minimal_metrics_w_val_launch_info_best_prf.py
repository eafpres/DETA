...
def parse_launch_script(script_path, log_path):
  """Extract selected training settings from a launch script.

  Args:
    script_path: Optional path to the shell launch script.
    log_path: Training-log path used for fallback experiment detection.

  Returns:
    dict: Selected experiment settings for display.
  """
  metadata = {
    "experiment": None,
    "batch_size": None,
  ...
  if script_path is not None:
    ...
    if not resolved_script_path.is_file():
      raise FileNotFoundError(
        f"launch script does not exist: {resolved_script_path}"
      )
    script_text = resolved_script_path.read_text(
      encoding = "utf-8",
      errors = "replace"
    )
  script_text = re.sub(r"\\\s*\n", " ", script_text)
  try:
    tokens = shlex.split(script_text, comments = True, posix = True)
  except ValueError:
  ...
  for index, token in enumerate(tokens[:-1]):
    if token in option_map:
      parsed[option_map[token]] = tokens[index + 1]
  for key in metadata:
    if key in parsed:
      metadata[key] = parsed[key]
  if metadata["finetune"]:
    metadata["finetune"] = Path(metadata["finetune"]).name
  experiment_sources = [
  ...
  for source in experiment_sources:
    if source is None:
      continue
    match = re.search(r"(?:^|[_-])(e\d+)(?:$|[_/.-])", source)
    if match is not None:
      metadata["experiment"] = match.group(1)
      break
  return metadata
def format_launch_metadata(metadata):
  """Format launch settings as a compact plot annotation.

  Args:
    metadata: Parsed launch-script settings.

  Returns:
    str: Multi-line annotation text.
  """
  def shown(value):
    return value if value not in (None, "") else "not set"
  return (
    f"experiment {shown(metadata['experiment'])}\n"
    f"batch size {shown(metadata['batch_size'])} | "
    f"queries {shown(metadata['num_queries'])}\n"
    f"lr drop {shown(metadata['lr_drop'])} | "
    f"gamma {shown(metadata['gamma'])}\n"
    f"dropout {shown(metadata['dropout'])} | "
    f"weight decay {shown(metadata['weight_decay'])}\n"
    f"finetune {shown(metadata['finetune'])}"
  )
...
def get_best_val_f1_records(val_f1_records):
  """Return the best comparable macro metric records.
  Args:
    val_f1_records: Parsed validation precision, recall, and F1 records.
  Returns:
    dict[str, dict] | None: Best record for each macro metric.
  """
  if not val_f1_records:
    return None
  reference = val_f1_records[-1]
  comparable_records = []
  for record in val_f1_records:
    same_mode = record["mode"] == reference["mode"]
    same_iou = abs(record["iou"] - reference["iou"]) < 1e-12
    if reference["threshold"] is None:
      same_threshold = record["threshold"] is None
    else:
      same_threshold = (
        record["threshold"] is not None and
        abs(record["threshold"] - reference["threshold"]) < 1e-12
      )
    if same_mode and same_iou and same_threshold:
...
class LossMonitorWindow(QtWidgets.QMainWindow):
  ...
  def __init__(self, args, log_path):
    ...
    self.launch_metadata = parse_launch_script(
    ...
    self.launch_text = self.ax.text(
      0.735,
      0.975,
      format_launch_metadata(self.launch_metadata),
    ...
    self.ax.legend(fontsize = 6, title_fontsize = 8, loc = "upper right")
  ...
  def update_plot(self):
    ...
    if self.val_f1_records:
      ...
      best_val_f1 = get_best_val_f1_records(self.val_f1_records)
      threshold_text = (
      ...
      self.val_f1_text.set_text(
        ...
        f"{'':<21}{'current':>9}{'best':>9}{'epoch':>7}\n"
        f"{'macro precision':<21}"
        f"{latest_val_f1['precision']:9.3f}"
        f"{best_val_f1['precision']['precision']:9.3f}"
        f"{best_val_f1['precision']['epoch'] + 1:7d}\n"
        f"{'macro recall':<21}"
        f"{latest_val_f1['recall']:9.3f}"
        f"{best_val_f1['recall']['recall']:9.3f}"
        f"{best_val_f1['recall']['epoch'] + 1:7d}\n"
        f"{'macro f1':<21}"
        f"{latest_val_f1['f1']:9.3f}"
        f"{best_val_f1['f1']['f1']:9.3f}"
        f"{best_val_f1['f1']['epoch'] + 1:7d}\n"
